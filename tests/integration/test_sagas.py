...
def test_skip_planning_accepts_current_task_settings_without_route_hash() -> None:
    """Current-task settings are not mistaken for a confirmed route response."""
    saga = MowPathSaga(
        command_builder=_make_command_builder(),
        send_command=AsyncMock(),
        get_map=HashList,
        zone_hashs=[],
        route_info=GenerateRouteInformation(one_hashs=[123]),
        skip_planning=True,
    )

    assert saga.result_path_hash == 0
...
def test_planning_fetches_complete_route_instead_of_reusing_preview() -> None:
    """A planned replacement never omits hashes present in the previous preview."""
    current_map = HashList()
    current_map.has_mow_path_for_hash = MagicMock(side_effect=lambda value: value == 100)
    builder = _make_command_builder()

    planning = MowPathSaga(builder, AsyncMock(), lambda: current_map, [], skip_planning=False)
    recovery = MowPathSaga(
        builder,
        AsyncMock(),
        lambda: current_map,
        [],
    ...
    assert planning._hashes_to_fetch([100, 200], current_map) == [100, 200]  # noqa: SLF001
    assert recovery._hashes_to_fetch([100, 200], current_map) == [200]  # noqa: SLF001
