...
async def test_inbound_callback_accepts_already_satisfied_report_token(
    rpt_handle: DeviceHandle,
) -> None:
    """The receive-loop guard does not hide telemetry already applied."""
    rpt_handle._report_data_generation = 2
    task = asyncio.current_task()
    assert task is not None
    rpt_handle._inbound_dispatch_tasks.add(task)
    try:
        assert await rpt_handle.wait_for_report_data(0.1, since=1)
    finally:
        rpt_handle._inbound_dispatch_tasks.discard(task)
...
async def test_failed_report_refresh_retry_is_delayed() -> None:
    """An unavailable transport cannot spin refresh attempts in one loop turn."""
    handle = make_handle()
    handle._report_refresh_deadline = time.monotonic() + 10
    handle._enqueue_report_refresh = AsyncMock()

    with patch("pymammotion.device.handle.asyncio.sleep", new_callable=AsyncMock) as sleep:
        await handle._retry_report_refresh()

    sleep.assert_awaited_once_with(1.0)
    handle._enqueue_report_refresh.assert_awaited_once_with()
...
async def test_broker_subscriber_cannot_wait_for_current_report() -> None:
    """An unsolicited broker callback cannot block report-state application."""
    from pymammotion.data.model.device import MowerDevice
    from pymammotion.proto import LubaMsg, MctlSys, ReportInfoData, RptDevStatus

    handle = DeviceHandle(
        device_id="dev-broker-freshness",
        device_name="Luba-Broker",
        initial_device=MowerDevice(name="Luba-Broker"),
    )
    results: list[bool] = []

    async def wait_from_broker(_message: object) -> None:
        results.append(
            await handle.wait_for_report_data(
                1.0, since=handle.report_data_token
            )
        )

    handle.broker.on_message = wait_from_broker
    msg = LubaMsg(
        sys=MctlSys(
    ...
    assert results == [False]
    assert handle.report_data_token == 1
...
async def test_report_during_timed_out_send_is_accepted(
    rpt_handle: DeviceHandle,
) -> None:
    """A report applied during a send timeout still proves stream freshness."""

    async def transport_send(_payload: bytes) -> None:
        _feed_report(rpt_handle)
        await asyncio.sleep(1)

    assert await rpt_handle._send_rpt_start_verified(  # noqa: SLF001
        b"cmd", transport_send, AsyncMock()
    )
