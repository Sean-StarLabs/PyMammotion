...
async def test_commit_mow_path_transactions_rejects_changed_active_path() -> None:
    """A late transfer cannot replace geometry for a newer route."""
    device = MowerDevice(name="Luba-Route", mow_session_id=2)
...
async def test_restored_native_only_trail_waits_for_report_verification() -> None:
    """A restored live trail without cover paths still needs session validation."""
    from pymammotion.proto import MctlSys, ReportInfoData, RptDevStatus, RptWork

    device = MowerDevice(name="Yuka-Restore", mow_session_id=3)
    device.report_data.dev.sys_status = WorkMode.MODE_WORKING
...
async def test_restored_active_route_accepts_split_report_verification() -> None:
    """Device and work evidence may arrive in consecutive report frames."""
    from pymammotion.proto import MctlSys, ReportInfoData, RptDevStatus, RptWork
...
async def test_commit_dynamics_line_keeps_longer_trail_on_shorter_same_session_poll() -> None:
    """A shorter same-session response cannot truncate retained native history."""
    device = MowerDevice(name="Luba-Route", mow_session_id=3)
    device.report_data.dev.sys_status = WorkMode.MODE_WORKING
    retained = [CommDataCouple(x=index, y=index) for index in range(3)]
    device.map.dynamics_line = retained
    device.map.dynamics_line_session_id = 3
    handle = DeviceHandle(device_id="dev-route", device_name=device.name, initial_device=device)

    accepted = await handle.commit_dynamics_line(retained[-2:], 3)

    assert accepted is True
    assert handle.snapshot.raw.map.dynamics_line == retained
    assert handle.snapshot.raw.map.dynamics_line_session_id == 3
