...
async def test_user_command_preempts_read_waiting_on_transport_gate() -> None:
    """A reconnect-gated read cannot block a later interactive command."""
    q = DeviceCommandQueue(device_name="dev-read-gate")
    broker = DeviceMessageBroker()
    order: list[str] = []
    ...
    try:
        ...
        for _ in range(10):
            if q._active_interruptible_task is not None:  # noqa: SLF001
                break
            await asyncio.sleep(0)
        assert q._active_interruptible_task is not None  # noqa: SLF001

        command = asyncio.create_task(
            q.run_after_preempting_reads(lambda: _append(order, "command"))
        )
        await asyncio.sleep(0)
...
async def test_gateway_timeout_before_offline_failure_does_not_restore_active_read() -> None:
    """A later definite failure cannot erase uncertainty from an earlier send."""
    q = DeviceCommandQueue(device_name="dev-timeout-then-offline")
    broker = DeviceMessageBroker()
    runs = 0
    started = asyncio.Event()
    ...
    async def fail_after_timeout() -> None:
        nonlocal attempts
        attempts += 1
        if attempts == 1:
            raise GatewayTimeoutException(20056, "iot-1")
        raise DeviceOfflineException(6205, "iot-1")
    ...
    try:
        ...
        with pytest.raises(DeviceOfflineException):
            await q.run_after_preempting_reads(fail_after_timeout)
        await asyncio.sleep(0.05)
...
async def test_completed_read_is_not_restored_while_its_callback_runs() -> None:
    """A completed read is not preemptible merely because its callback is slow."""
    q = DeviceCommandQueue(device_name="dev-completed-read")
    broker = DeviceMessageBroker()
    callback_started = asyncio.Event()
    release_callback = asyncio.Event()
    runs = 0

    class _CompletedRead(Saga):
        name = "completed_read"
        interruptible = True
    ...
    q.start()
    try:
        await q.enqueue_saga(_CompletedRead(), broker, on_complete=on_complete)
        await asyncio.wait_for(callback_started.wait(), timeout=2.0)

        command = asyncio.create_task(q.run_after_preempting_reads(offline))
        await asyncio.sleep(0)
        release_callback.set()
        with pytest.raises(DeviceOfflineException):
            await command
...
async def test_uncertain_command_timeout_invalidates_older_queued_read() -> None:
    """A transmitted command timeout invalidates reads with stale assumptions."""
    q = DeviceCommandQueue(device_name="dev-timeout-queued-read")
    broker = DeviceMessageBroker()
    order: list[str] = []
    ...
    await q.enqueue_saga(_TrackingReadSaga(order, "read"), broker)
    command = asyncio.create_task(q.run_after_preempting_reads(timeout))
    await asyncio.sleep(0)
    q.start()
    try:
        with pytest.raises(GatewayTimeoutException):
