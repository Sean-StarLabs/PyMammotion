...
async def test_report_data_token_returns_handle_token() -> None:
    ...
    client = MammotionClient()
    handle = make_handle("dev1", "Yuka-Prime")
    handle._report_data_generation = 42
    await client._device_registry.register(handle)

    assert client.report_data_token("Yuka-Prime") == 42
    assert client.report_data_token("no-such-device") == 0
...
async def test_start_mow_path_saga_respects_disabled_option_over_ble() -> None:
    ...
    with patch("pymammotion.client.MowPathSaga") as mock_saga:
        await client.start_mow_path_saga("Luba-Mow", zone_hashs=[1, 2], skip_planning=True)
...
async def test_disabled_path_fetch_does_not_block_route_planning() -> None:
    """The path-fetch preference must not suppress a requested route plan."""
    client = MammotionClient()
    handle = await _make_handle_with_transport("dev1", "Luba-Plan")
    handle.set_mow_path_fetch_enabled(value=False)
    handle.enqueue_saga = AsyncMock()  # type: ignore[method-assign]
    await client._device_registry.register(handle)

    with patch("pymammotion.client.MowPathSaga") as mock_saga:
        await client.start_mow_path_saga("Luba-Plan", zone_hashs=[1, 2])
