...
from pymammotion.data.model.hash_list import MowPath, MowPathPacket, RootHashList
...
async def test_start_mow_path_saga_commits_result_on_completion() -> None:
    ...
    with patch("pymammotion.client.MowPathSaga") as MockSaga:
        ...
        mock_saga_instance.started_path_hash = 456
        MockSaga.return_value = mock_saga_instance
...
async def test_running_mow_path_binds_session_when_saga_executes() -> None:
    ...
    with patch("pymammotion.client.MowPathSaga") as mock_saga:
        ...
        async def execute(_broker: object) -> None:
            ...
            saga.started_path_hash = saga_call.kwargs["get_active_path_hash"]()
...
async def test_cached_session_with_different_path_hash_is_refetched() -> None:
    """A session match alone cannot certify geometry for a changed route."""
    client = MammotionClient()
    handle = await _make_mower_handle_with_transport("dev1", "Luba-New-Route")
    await client._device_registry.register(handle)
    device = handle.snapshot.raw
    device.mow_session_id = 3
    device.report_data.work.path_hash = 456
    ...
    with patch("pymammotion.client._should_fetch_mow_path", return_value=True):
        await client.check_and_get_mow_path(device.name)
