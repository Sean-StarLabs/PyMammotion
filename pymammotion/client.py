...
class MammotionClient:
        ...
        * ``init_cfg_hash`` (from ``report_data.work.init_cfg_hash``) — fires
        ...
        async def _on_path_changed(identity: tuple[int, int]) -> None:
            ...
            if path_hash in (0, 1):
                return
            if (
                device.map.has_mow_path_for_session(session_id)
                and device.map.has_mow_path_for_hash(path_hash)
            ):
    ...
    async def check_and_get_mow_path(self, device_name: str) -> None:
        ...
        if handle := self._device_registry.get_by_name(device_name):
            ...
            if (
                device.map.has_mow_path_for_session(device.mow_session_id)
                and device.map.has_mow_path_for_hash(path_hash)
            ):
                return  # Cache is valid for the current route
            if not _should_fetch_mow_path(device, path_hash):
        ...
        if handle := self._device_registry.get_by_name(device_name):
            ...
            def _active_path_hash() -> int:
                ...
                if not isinstance(current, MowerDevice):
                    return 0
                if current.report_data.dev.sys_status not in MOWING_ACTIVE_MODES:
                    return 0
            ...
            async def _on_mow_path_complete() -> None:
                ...
                committed = await handle.commit_mow_path_transactions(
                    ...
                    expected_path_hash=saga.started_path_hash if skip_planning else 0,
    ...
    async def get_dynamics_line(self, device_name: str) -> None:
        ...
        def _mow_session_id() -> int:
            device = handle.snapshot.raw
            return device.mow_session_id if isinstance(device, MowerDevice) else 0
