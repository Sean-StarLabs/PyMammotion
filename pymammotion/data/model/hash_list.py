...
class HashList(DataClassORJSONMixin):
    ...
    """
    dynamics_line_session_id: int = 0
    """Locally assigned mowing session associated with ``dynamics_line``."""
    ...
    def update_dynamics_line(self, points: list[CommDataCouple], session_id: int) -> None:
        """Replace ``dynamics_line`` with *points*.

        Callers retain the longest accepted snapshot within a mowing session;
        this method replaces that accepted snapshot atomically.
        """
        self.dynamics_line = points
        self.dynamics_line_session_id = session_id

    def clear_dynamics_line(self) -> None:
        """Clear task-bound live route geometry."""
        self.dynamics_line = []
        self.dynamics_line_session_id = 0
    ...
    def has_native_trail_for_session(self, session_id: int) -> bool:
        """Return whether either native trail source belongs to *session_id*."""
        return self.has_mow_path_for_session(session_id) or bool(
            session_id
            and self.dynamics_line
            and self.dynamics_line_session_id == session_id
        )
