...
class _QueueItem:
    ...
    delivery_uncertain: asyncio.Event | None = field(compare=False, default=None)
    interruptible_preempt_event: asyncio.Event | None = field(compare=False, default=None)
    interruptible_requeue: Callable[[], Awaitable[None]] | None = field(
...
class DeviceCommandQueue:
    ...
    def __init__(self, device_name: str = "") -> None:
        ...
        self._dequeued_interruptible_preempt_event: asyncio.Event | None = None
        self._dequeued_interruptible_requeue: Callable[[], Awaitable[None]] | None = None
    ...
    async def enqueue(
        ...
        *,
        ...
        delivery_uncertain: asyncio.Event | None = None,
        interruptible_preempt_event: asyncio.Event | None = None,
        interruptible_requeue: Callable[[], Awaitable[None]] | None = None,
    ) -> None:
        ...
        item = _QueueItem(
            ...
            interruptible_preempt_event=interruptible_preempt_event,
            interruptible_requeue=interruptible_requeue,
        )
    ...
    async def enqueue_saga(
        ...
        *,
        ...
        preempt_event = asyncio.Event() if saga.interruptible else None

        async def _requeue() -> None:
            await self.enqueue_saga(
                saga,
                broker,
                on_complete,
            ...
            try:
                ...
                if saga.interruptible:
                    ...
                    if saga.restore_on_failed_preemption:
                        self._active_interruptible_requeue = _requeue
                    else:
                ...
                if not saga_completed:
                    return
                # The read is complete before its callback starts. A later command
                # must not mistake this dequeued item for a preempted read and run it
                # again if the command cannot be delivered.
                if saga.interruptible and self._dequeued_interruptible_preempt_event is preempt_event:
                    self._dequeued_interruptible_preempt_event = None
                    self._dequeued_interruptible_requeue = None
        ...
        await self.enqueue(
            ...
            expires=False,
            interruptible_preempt_event=preempt_event,
            interruptible_requeue=_requeue if saga.restore_on_failed_preemption else None,
        )
    ...
    async def run_after_preempting_reads(self, work: Callable[[], Awaitable[None]]) -> None:
        ...
        delivery_uncertain = asyncio.Event()
        ...
        restore_read = (
            self._active_interruptible_requeue
            or self._dequeued_interruptible_requeue
        )
        if self._dequeued_interruptible_preempt_event is not None:
            self._dequeued_interruptible_preempt_event.set()
        ...
        try:
            ...
            if dispatch_started.is_set() and (delivery_uncertain.is_set() or not delivery_was_impossible):
            ...
            if (
                ...
                and (not dispatch_started.is_set() or (delivery_was_impossible and not delivery_uncertain.is_set()))
            ):
                try:
    ...
    async def _execute_item(self, item: _QueueItem) -> None:
        ...
        for attempt in range(1, _GATEWAY_TIMEOUT_MAX + 1):
            ...
            try:
                ...
                if item.delivery_uncertain is not None:
                    item.delivery_uncertain.set()
    ...
    async def _process(self) -> None:
        ...
        while self._running:
            ...
            try:
                ...
                if item.priority > Priority.EMERGENCY and item.priority != Priority.INTERRUPTIBLE_READ:
                    await self._transport_gate.wait()
                    if item.completion is not None and item.completion.cancelled():
                        continue

                if item.interruptible_preempt_event is not None:
                    self._dequeued_interruptible_preempt_event = (
                        item.interruptible_preempt_event
                    )
                    self._dequeued_interruptible_requeue = item.interruptible_requeue
                try:
                    await self._execute_item(item)
                finally:
                    if (
                        self._dequeued_interruptible_preempt_event
                        is item.interruptible_preempt_event
                    ):
                        self._dequeued_interruptible_preempt_event = None
                        self._dequeued_interruptible_requeue = None
            except asyncio.CancelledError:
