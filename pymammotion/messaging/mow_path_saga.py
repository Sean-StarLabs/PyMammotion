...
class MowPathSaga(Saga):
    ...
    def _capture_task_identity(self) -> None:
        ...
        self.started_path_hash = self._get_active_path_hash() if self._get_active_path_hash is not None else 0
    ...
    async def _fetch_line_hash_list(
        ...
        if self.result_root_hash_list.data and not self._force_refresh:
    ...
    async def _run(self, broker: DeviceMessageBroker) -> None:
        ...
        if self._skip_planning:
            if (
                (self._get_mow_session_id is not None and self.result_session_id == 0)
                or (
                    self._get_active_path_hash is not None
                    and self.started_path_hash in (0, 1)
                )
            ):
        ...
        self.result_root_hash_list = await self._fetch_line_hash_list(broker)
