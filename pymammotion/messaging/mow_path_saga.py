...
class MowPathSaga(Saga):
    ...
    def __init__(
        ...
        *,
        ...
        get_active_path_hash: Callable[[], int] | None = None,
    ) -> None:
        """Initialise the saga.
        ...
        self._get_active_path_hash = get_active_path_hash
    ...
    def _capture_task_identity(self) -> bool:
        """Bind a fetch-only read to the active task at dispatch time."""
        if not self._skip_planning or self._get_active_path_hash is None:
            return True
        if not self._task_identity_captured:
            self.started_path_hash = self._get_active_path_hash()
            self._task_identity_captured = True
        return self.started_path_hash not in (0, 1)
    ...
    def _store_batch_frame(
        ...
        if expected_hashes is not None and any(
            int(packet.path_hash) not in expected_hashes
            for packet in frame.path_packets
        ):
            raise SagaFailedError(MowPathSaga.name, MowPathSaga.max_attempts)
        ...
        if complete and expected_hashes is not None:
            observed_hashes = {
                int(packet.path_hash)
                for stored_frame in frames.values()
                for packet in stored_frame.path_packets
            }
    ...
    async def _run(self, broker: DeviceMessageBroker) -> None:
        ...
        if not self._capture_task_identity():
            _logger.debug("MowPathSaga: active task ended before route fetch dispatch")
            return
        self.result = {}
