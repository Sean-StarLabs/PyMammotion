...
async def dynamics_line_loop(handle: DeviceHandle) -> None:
    ...
    while not handle._stopping:  # noqa: SLF001
        # The dedicated event only tracks transport topology. The handle's shared
        # rearm event is set after every saga and would make this loop wake itself.
        prefer_ble = _ble_connected(handle)
        await _sleep_until_transport_change(
            handle,
        ...
        if not prefer_ble and not _cloud_connected(handle):
            continue
        if not prefer_ble and Transport.firmware_uses_send_quota(
            handle.firmware_version
        ):
            # A 60-second poll still exceeds the 600 sends/12h allowance before
            # response acknowledgements are counted. Wait for BLE or exempt
            # firmware rather than consuming the robot's command budget.
            continue
...
async def _sleep_until_transport_change(handle: DeviceHandle, timeout: float) -> None:
    """Wait for the next poll or wake early when BLE availability changes."""
    event = handle._dynamics_line_rearm_event  # noqa: SLF001
    event.clear()
