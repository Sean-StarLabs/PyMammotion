...
#: Prevent an immediately failing transport from spinning freshness retries.
_REPORT_REFRESH_RETRY_DELAY: float = 1.0
...
class DeviceHandle:
    ...
    async def on_raw_message(self, payload: bytes, transport_type: TransportType = TransportType.CLOUD_ALIYUN) -> None:
        ...
        already_guarded = (
            ...
            and dispatch_task in self._inbound_dispatch_tasks
        )
        ...
        try:
            ...
            if dispatch_task is not None and not already_guarded:
        ...
        if nav is not None and (
            ...
            await self._emit_inbound_event(self._map_updated_bus, None)
        ...
        if luba_msg.sys is not None and luba_msg.sys.mow_to_app_info is not None:
            ...
            if info.type == 0 and info.cmd == 0 and info.mow_data:
                ...
                await self._emit_inbound_event(
                    self._shutdown_bus,
    ...
    async def on_status_message(self, msg: ThingStatusMessage) -> None:
        ...
        if not self._stopping:
            await self.emit_state_changed(snapshot)
            await self._emit_inbound_event(self._status_bus, msg)
    ...
    async def on_device_event(self, event: ThingEventMessage) -> None:
        ...
        if isinstance(event.params, DeviceProtobufMsgEventParams):
            ...
            if not self._stopping:
                await self.emit_state_changed(snapshot)
                await self._emit_inbound_event(self._event_bus, event)
    ...
    async def on_device_properties(self, properties: ThingPropertiesMessage) -> None:
        ...
        if not self._stopping:
            await self.emit_state_changed(snapshot)
            await self._emit_inbound_event(self._properties_bus, properties)
    ...
    def update_availability(
        ...
        *,
        ...
        if old_state != new_state and not self._stopping:
            ...
            asyncio.get_running_loop().create_task(self.emit_state_changed(snapshot))
        ...
        *,
        ...
        for attempt in (1, 2):
            ...
            try:
                ...
                if self._report_data_generation > before:
                    return True
                continue
        ...
        async def _send() -> None:
            ...
            try:
                if self._report_refresh_deadline <= time.monotonic():
                    return
                if self._report_data_generation > self._report_refresh_required_generation:
                    return
                ble = self._transports.get(TransportType.BLE)
                if self._ble_stream_active and ble is not None and ble.is_connected:
                    ...
                    refresh_succeeded = await self._send_rpt_start_verified(
                        stream_cmd,
                        _ble_send,
                        _ble_sync,
                ...
                if (
                    ...
                    schedule = (
                        self._schedule_report_refresh_now
                        if refresh_succeeded
                        else self._schedule_report_refresh_retry
                    )
    ...
    def _schedule_report_refresh_retry(self) -> None:
        ...
        if self._report_refresh_retry_task is not None and not self._report_refresh_retry_task.done():
            return
        self._report_refresh_retry_task = asyncio.create_task(
            self._retry_report_refresh()
        )

    def _schedule_report_refresh_now(self) -> None:
        """Queue a follow-up immediately when a later waiter missed a response."""
        if self._stopping:
            return
        if (
            self._report_refresh_retry_task is not None
            and not self._report_refresh_retry_task.done()
        ):
            return
        self._report_refresh_retry_task = asyncio.create_task(
            self._enqueue_report_refresh()
        )

    async def _retry_report_refresh(self) -> None:
        """Retry a stale report refresh after a bounded failure delay."""
        remaining = self._report_refresh_deadline - time.monotonic()
        if remaining <= 0:
            return
        await asyncio.sleep(min(_REPORT_REFRESH_RETRY_DELAY, remaining))
        await self._enqueue_report_refresh()
    ...
    async def emit_state_changed(self, snapshot: DeviceSnapshot) -> None:
        ...
        if self._stopping:
            return
        await self._emit_inbound_event(self._state_changed_bus, snapshot)

    async def _emit_inbound_event(self, bus: Any, value: Any) -> None:
        """Emit an inbound callback without allowing receive-loop waits."""
        dispatch_task = asyncio.current_task()
        already_guarded = (
            dispatch_task is not None
            and dispatch_task in self._inbound_dispatch_tasks
