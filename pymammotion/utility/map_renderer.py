...
def _render_tile_source(
    ...
    max_tile_x = min(math.floor((source_min_x + source_width) / OSM_TILE_SIZE), max_tile)
    min_tile_y = max(math.floor(source_min_y / OSM_TILE_SIZE), 0)
    max_tile_y = min(math.floor((source_min_y + source_height) / OSM_TILE_SIZE), max_tile)
    ...
    for tile_x in range(min_tile_x, max_tile_x + 1):
        for tile_y in range(min_tile_y, max_tile_y + 1):
            ...
            dest_x = round(tile_x * OSM_TILE_SIZE - source_min_x)
            dest_y = round(tile_y * OSM_TILE_SIZE - source_min_y)
            # paste() clips when dest is negative or the tile extends past the canvas,
            # unlike alpha_composite() which raises ValueError for out-of-bounds dest.
            source.paste(tile_rgba, (dest_x, dest_y), tile_rgba)
    return source
...
def _load_tile(
    ...
    if cache_path:
        try:
            ...
            # Atomic write via temp-file rename so concurrent renders never read a
            # partially-written PNG, and a failed write never corrupts the cache.
            with tempfile.NamedTemporaryFile(dir=cache_path.parent, suffix=".tmp", delete=False) as f:
                f.write(tile_bytes)
                tmp_path = f.name
            Path(tmp_path).replace(cache_path)
        except OSError:
            pass  # non-fatal — tile is still returned from memory below
...
def _draw_geojson_feature(draw: ImageDraw.ImageDraw, feature: dict[str, Any], project) -> None:
    ...
    if geometry_type == "Polygon":
        ...
        all_outer_pts: list[tuple[float, float]] = []
        for polygon_coordinates in geometry.get("coordinates", []):
            for i, ring in enumerate(polygon_coordinates):
                polygon = [project((float(coord[0]), float(coord[1]))) for coord in ring if len(coord) >= 2]
                if len(polygon) >= 3:
                    draw.polygon(polygon, fill=fill, outline=stroke)
                    draw.line([*polygon, polygon[0]], fill=stroke, width=3, joint="curve")
        ...
        if name and type_name == "area":
            _draw_label(draw, str(name), _centroid(all_outer_pts), project)
    elif geometry_type in {"LineString", "MultiLineString"}:
        lines = [geometry.get("coordinates", [])] if geometry_type == "LineString" else geometry.get("coordinates", [])
        for coordinates in lines:
            line = [project((float(coord[0]), float(coord[1]))) for coord in coordinates if len(coord) >= 2]
            if len(line) >= 2:
...
def _parse_colour(value: Any, fallback: tuple[int, int, int, int]) -> tuple[int, int, int, int]:
    if not value:
        return fallback
    try:
        return cast("tuple[int, int, int, int]", ImageColor.getcolor(str(value), "RGBA"))
    except ValueError:
        return fallback
...
def _draw_text(draw: ImageDraw.ImageDraw, text: str, xy: tuple[float, float]) -> None:
    ...
    x, y = xy
    padding = 4
    # Box is anchored to the actual glyph bounding box so padding is uniform on all sides.
    draw.rounded_rectangle(
        (
            x + text_bounds[0] - padding,
            y + text_bounds[1] - padding,
            x + text_bounds[2] + padding,
            y + text_bounds[3] + padding,
        ),
        radius=5,
...
def _geometry_points(value: Any) -> list[tuple[float, float]]:
    ...
    if geometry_type == "FeatureCollection":
        return [point for feature in value.get("features", []) for point in _geometry_points(feature)]
    if geometry_type == "Feature":
        return _geometry_points(value.get("geometry") or {})
    if geometry_type == "GeometryCollection":
        return [point for geometry in value.get("geometries", []) for point in _geometry_points(geometry)]
